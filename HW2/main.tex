...
\begin{parts}
    ...
    对于这个式子我们分开求导:\\
    \[
        \frac{\partial LL(D_l)}{\partial \mu_i}  = \sum_{(\boldsymbol{x}_j, y_j) \in D_l \wedge y_j = i}\frac{1}{p(x_j|\boldsymbol{\mu_i}, \boldsymbol{\Sigma_i})}\cdot\frac{p(x_j|\boldsymbol{\mu_i}, \boldsymbol{\Sigma_i})}{\mu_i}= \sum_{(\boldsymbol{x}_j, y_j) \in D_l \wedge y_j = i} \Sigma_i^{-1}(x_j - \mu_i)
    \]
    \[
